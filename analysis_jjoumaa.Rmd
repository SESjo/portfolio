...
```{r missingValue, fig.cap="Representation of missing values within the learning dataset", fig.align="center"}
# build dataPlot
...
```{r densityHistogram, fig.cap="Histogram and density for each of the numerical attributes.", fig.align="center"}
...
```{r boxplot, fig.cap="Boxplot for each of the numerical attributes.", fig.align="center"}
ggplot(melt(dataset[, .SD, .SDcols = c(ncolInt, ncol(dataset))], id.vars = "Y"), 
...
```{r nominalAttributes, echo=FALSE, fig.align="center", fig.cap="Barplot for each of the nominal attributes."}
...
When looking at Figure \@ref(fig:missingValue), it appears that a lot of missing values occurred within the learning dataset. Here, I've taken the liberty to "simply" removed these values. Another way to deal with missing values would have been to use an imputation method, such as the [K-nearest neighbors method](https://topepo.github.io/caret/pre-processing.html#imputation) [^1] provided by the package `caret` (code commented below). Because this method requires high computational costs, I've preferred to stick to the first method, even if, as we'll see in the next steps, it means the final model won't be able to predict `Y` in some cases.
...
```{r correlation, fig.cap="Correlogram of dataset.", fig.align="center"}
# matrix correlation
...
I've used the Yeo-Johnson transform, since the attributes "AAGE", *i.e* the age of the people in this dataset, has a Gaussian-like distribution with a skew (Figure \@ref(fig:densityHistogram). To make it "more Gaussian", I've simply performed a `YeoJohnson` transform; I would have normally performed a `BoxCox` transform, but it does not support raw values that are equal to zero.
...
```{r dotplot, fig.align="center", fig.cap="Models comparison"}
dotplot(results)
```
...
```{r model_tuning, echo=TRUE, eval=FALSE}
...
```{r correlogramPrediction, fig.align="center", fig.cap="Correlogram of the predictions by models."}
...
```{r varImportance, fig.align="center", fig.cap="Distribution and barplot of the three most important variables when predicting the income level of the person represented by the record."}
